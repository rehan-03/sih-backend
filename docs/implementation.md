...
- [ ] Write `contracts/openapi.yaml` for every route (with Frontend Owner)
- [ ] Agree `contracts/entities.md` — `Wallet`, `Complaint`, `Case`, `Alert`, `RiskEvidence`, and the closed enums (`risk_tier`, case `status`, alert `action`)
- [ ] Define the standard error envelope and apply it everywhere
- [ ] Scaffold FastAPI app: `main.py`, `core/`, `api/v1/routers/`, `deps.py`
- [ ] `docker-compose.yml`: postgres + neo4j + redis + backend
- [ ] `/health` returns 200
- [ ] JWT scaffolding (`/auth/login`, `/auth/refresh`) — shape only, real RBAC in Phase 2
...
- [ ] `complaints` + `complaint_wallets` tables + Alembic migration
- [ ] Synthetic-NCRP generator script (controlled shared-wallet duplication for the demo)
- [ ] Deterministic correlation scoring (exact wallet match; fuzzy record-linkage optional later)
...
- [ ] Redis key shape: `risk:{chain}:{address} → {score, tier, case_ref, flagged_at, ttl}`
- [ ] `POST /check-wallet` — API-key auth (not JWT), Redis-only hot path, p95 < 200ms
- [ ] Mock VASP client simulating incoming deposits
- [ ] `alerts` table + `GET /api/v1/alerts`
- [ ] Celery task: on a hold decision, enqueue `alert_service.notify()` off the response path
...
- [ ] BTC + ETH (+ TRON if time allows) explorer API integrations
- [ ] Neo4j graph builder as a Celery async job (`Wallet`/`Transaction`/`VASP`/`Cluster`)
- [ ] Nearest-VASP Cypher query (shortest `SENT→RECEIVED_BY→DEPOSITS_TO` path)
- [ ] `GET /api/v1/wallets/{address}/trace`

## Phase 4 — ML Risk Scoring
- [ ] Feature engineering: in/out-degree, tx velocity, volume in/out, wallet age, hop-distance to known-illicit cluster, correlation score (from Phase 1), fan-in/out counts, mixer-proximity flag, cross-chain bridge flag, avg tx value, time-of-day anomaly
- [ ] Train XGBoost/LightGBM baseline on Elliptic/Elliptic++ (temporal split — see `datasets-and-ml.md`)
- [ ] SHAP explainability wired into the evidence array shape
- [ ] `GET /api/v1/wallets/{address}/risk`
- [ ] Registry-refresh job materializing model output into `risk:{chain}:{address}`

## Phase 5 — Case Management + Reports
- [ ] `cases` + `case_wallets` tables
- [ ] `PATCH /api/v1/cases/{id}`
- [ ] PDF report generation (case summary, evidence chain, linked complaints)
- [ ] `GET /api/v1/cases/{id}/report`

## Phase 6 — LLM NER (USP 3, stretch — local air-gapped extraction)
- [ ] Ollama + Llama-3.2-3B-Instruct locally, air-gapped (FIR text never leaves the intranet except to local Ollama; fits in 4GB GPU VRAM with sub-1.5s latency)
- [ ] Structured JSON extraction prompt (names, amounts, wallets, dates, typology)
- [ ] spaCy fallback/validator
- [ ] Expose extracted entities read-only on the complaint-detail response — no new endpoint
...
- [ ] End-to-end run against the real backend (no mocks) for every screen
- [ ] Error/loading states, CORS configured
- [ ] `/check-wallet` load test — confirm p95 < 200ms
- [ ] `docker-compose up` as the single demo-day command
- [ ] Audit log verified on every case view/export
